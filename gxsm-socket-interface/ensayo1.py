...
##### Start putting in libraries that I need
import sys  
import os		# use os because python IO is bugy
import time
import threading

from netCDF4 import Dataset, chartostring, stringtochar, stringtoarr #### These are used for image loading
import struct
import array
import math
...
#import cairo ##### This is for the gui stuff 

##### next up is matlab stuff
import scipy.misc
...
########This is the socket client to connect to GXSM
...
########## App Elements, Title and menu titles
...
class AppWindow(Gtk.ApplicationWindow):
 ...
 ## IMAGEPARAMETERS
        ...
        buttonG6 = Gtk.Button(label="Normalize")
        buttonG6.connect("clicked", self.normalize_clicked)
        grid.attach(buttonG6, c, y, 2, 1)
        y=y+1

        buttonG7 = Gtk.Button(label="Blurry")
        buttonG7.connect("clicked", self.blurry_clicked)
        grid.attach(buttonG7, c, y, 2, 1)
        y=y+1
    ...
    def normalize_clicked(self, widget):
        self.ImageData *= (255.0/self.ImageData.max()) # scale to unit
    ...
    def blurry_clicked(self, widget):
        self.ImageData = ndimage.gaussian_filter(self.ImageData, sigma=3)
