####### Fifth Version of AI: Version as of October 31st 2022
...
#Resgister the data. Where is it located locally? Change location to where files are on your system. 
...
#Add labels from the instances registered. 
...
# Load weights from model and decide on Threshold. Run predictor. Make sure to change where the file is located on your system. Threshold can be changed to make model more selective
...
#Select what functions you would like to run
#auto_locate to use CV to find molecules automatically. Works well on flat areas with no steps.
#stm images in STM mode and centers on the molecule.
#afm takes an initial z ref point and then images in HR-AFM mode.
#AI_afm adds the option to optimize the HR-AFM imaging. Takes the initial afm image and then decides to either move on to the next molecule, take a step closer and take one more image, or take a step away and take one more image.  
...
def auto_afm_scanspeed(y):
	ms = gxsm.get_slice(afm_ch, 0,0, y,1) # ch, v, t, yi, yn   ## AFM dFreq in CH3
	med = np.median(ms)
